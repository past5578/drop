...
The main endpoints to worry about are `/i/v?id={image_id}`, `/i/r?id={image_id}`, and `/i/upload` which provide the pretty HTML page for an image, and provide the raw image respectively. Eventually, I would like to turn this into a full on website with users and everything of the sort, but that will be in the future.
